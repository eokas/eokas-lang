...
const char* const token_t::names[] = {
	"echo", "var", "val", "make", "schema", "struct", "proc", "func",
...
void scanner_t::scan_string(char delimiter)
{
	...
	while (m_current != delimiter)
	{
		...
		else if (m_current == '\\') // eseokas sequence
		{
			...
			switch (m_current)
			{
			case 'a': this->save_char('\a'); this->read_char(); break;
			case 'b': this->save_char('\b'); this->read_char(); break;
			case 'f': this->save_char('\f'); this->read_char(); break;
			case 'n': this->save_char('\n'); this->read_char(); break;
			case 'r': this->save_char('\r'); this->read_char(); break;
			case 't': this->save_char('\t'); this->read_char(); break;
			case 'v': this->save_char('\v'); this->read_char(); break;
			case 'x': // \xFF
				this->read_char();
