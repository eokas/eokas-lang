...
namespace eokas
{
	...
	bool llvm_module_t::add_type(const String& name, llvm_type_t* type)
	{
	...
	bool llvm_module_t::add_value(const String& name, llvm_value_t* value)
	{
		if(value == nullptr)
			return false;
	...
	llvm_value_t* llvm_module_t::get_value(const String& name)
	{
		return this->scope->get_value(name, false);
	}
	...
	llvm_type_t::llvm_type_t(llvm_module_t* module, const String& name)
		: module(module)
		, name(name)
		, handle(nullptr)
		, members()
	{
		this->handle = llvm::StructType::create(module->context, name.cstr());
	}
	...
	llvm_value_t::llvm_value_t(llvm_module_t* module)
		: module(module)
		, value(nullptr)
	{}
		...
		, IR(module->context)
	{
		...
		this->func->setAttributes(attrs);
		this->func->setCallingConv(llvm::CallingConv::C);
	...
	{
		llvm::BasicBlock* bb = llvm::BasicBlock::Create(module->context, name.cstr(), this->func);
		return bb;
	}
	...
	void llvm_function_t::add_tail_ret()
	{
		...
		{
			auto retT = this->func->getReturnType();
			if(retT->isVoidTy())
				IR.CreateRetVoid();
			else
