...
struct llvm_coder_t {
    ...
    llvm_coder_t(llvm::LLVMContext &llvm_context)
        ...
        type_string = llvm_define_type_string(llvm_context, llvm_module);
    ...
    llvm::Value *encode_expr_string(struct ast_expr_string_t *node) {
        ...
        llvm::StructType *stringType = llvm::cast<llvm::StructType>(this->scope->getType("string", true));
        if (stringType == nullptr)
            return nullptr;

        llvm::Value *stringValue = llvm_builder.CreateAlloca(stringType);
        llvm::Value *rawstr = llvm_builder.CreateGlobalString(node->value.cstr());
        llvm::Value *rawptr = llvm_builder.CreateStructGEP(stringValue, 0);
        llvm_builder.CreateStore(rawstr, rawptr);
